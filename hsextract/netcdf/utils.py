        ...
        if (
            ...
            if projection_import_string_dict.get("type") == "Proj4 String":
                try:
                    ...
                    box_info["northlimit"], box_info["westlimit"] = (
                        transformer.transform(
                            original_box_info["westlimit"],
                            original_box_info["northlimit"],
                        )
                    )
                    box_info["southlimit"], box_info["eastlimit"] = (
                        transformer.transform(
                ...
                try:
                    ...
                    box_info["westlimit"], box_info["northlimit"] = (
                    ...
                    box_info["eastlimit"], box_info["southlimit"] = (
                        crs_transform.TransformPoint(
...
def get_type_specific_meta(nc_dataset):
    ...
    dimensions = get_nc_dimensions(nc_dataset)

    # get netcfd coordinates
    coordinates = get_nc_coordinates(nc_dataset)

    # get netcdf variables
    nc_data_variables = nc_dataset.variables  # get_nc_data_variables(nc_dataset)
    type_specific_meta = extract_nc_data_variables_meta(
        nc_data_variables, nc_dimensions=[d["name"] for d in dimensions]
    )
...
def extract_nc_data_variables_meta(nc_data_variables, nc_dimensions=[]):
    """
    (dict) -> dict
    ...
    for var_name, var_obj in list(nc_data_variables.items()):
        # skip variables that are dimensions because these are represented as coordinates
        if var_name in nc_dimensions:
            continue
        nc_data_variables_meta[var_name] = {
            ...
            "shape": (
                ",".join(var_obj.dimensions) if var_obj.dimensions else "Not defined"
            ),
            "descriptive_name": (
                var_obj.long_name if hasattr(var_obj, "long_name") else ""
            ),
            "missing_value": str(
...
def get_nc_coordinates(nc_file_name):
    ...
    coordinate_meta = []
    for dim in nc_dataset.dimensions.keys():
        if dim in nc_dataset.variables:
            v = nc_dataset.variables[dim]
            coordinate_meta.append(
                dict(
                    name=v.name,
                    description=getattr(v, "long_name", None),
                    unit=getattr(v, "units", None),
                    dimensions=[dim],
                )
            )
    return coordinate_meta


def get_nc_dimensions(nc_file_name):
    ...
    for dim in nc_dataset.dimensions.keys():
        if dim in nc_dataset.variables:
            ...
            dimension_meta.append(dict(name=dim, shape=nc_dataset.dimensions[dim].size))
    ...
    for variables_type, variables_dict in list(nc_variables_dict.items()):
        for var_name, var_obj in list(variables_dict.items()):
            ...
            if hasattr(var_obj, "bounds") and nc_dataset.variables.get(
                ...
                nc_variables_coordinate_type_mapping[var_obj.bounds] = (
...
def add_temporal_coverage_metadata(extracted_metadata):
    ...
    if extracted_metadata.get("period"):
        start = dateutil.parser.isoparse(
            extracted_metadata["period"]["start"]
        ).isoformat()
...
def combine_metadata(extracted_core_meta, extracted_specific_meta):
    ...
    # add dimensions, coordinates, and variables
    metadata.update(extracted_specific_meta)

    ## add variables
    # variables = add_variable_metadata(extracted_specific_meta["variables"])
    # if variables:
    ...
    # pdb.set_trace()
    ## add dimensions, coordinates, and variables
    ## This can be combined with the previous step in the future.
    # metadata.update(extracted_specific_meta["dimensions"])
    # metadata.update(extracted_specific_meta["coordinates"])
